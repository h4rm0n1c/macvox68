...
- The formatter in `vox_parser.c` mirrors the upstream `vox_process` stages (sentence split → “thee” heuristic → lead-in break → time/number/degree handling → beat building → letter normalization → tidy/wrap) before emitting Speech Manager commands. FlexTalk emission is no longer exposed; the parser always translates markers into Speech Manager metadata.
