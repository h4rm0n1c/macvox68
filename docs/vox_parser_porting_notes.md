...
- A starter formatter now exists as `vox_format_text` (see `vox_parser.c`). It takes an ASCII buffer, emits `[[vers 1]]` when requested, and converts common FlexTalk markers such as `\!br` and `\!sfNNN` into `[[slnc …]]` pauses. Future work: replace the pass-through sections with the full beat-building pipeline so this function becomes the single entry point for speech text in MacVox68.
