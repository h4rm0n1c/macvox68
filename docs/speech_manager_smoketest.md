...
- The Retro68 target must link against the Speech Manager import archive so `InitSpeech`, `SpeechBusySystemTask`, and `StopSpeechChannel` resolve. MPW 3.x era kits ship 68K stubs as `Speech.a` or `SpeechSynthesis.a`; point CMake at the folder containing one of those via `-DSPEECH_IMPORT_LIB_DIR=/path/to/ImportLibraries`. If CMake reports the library missing, check both `ImportLibraries` and `Interfaces&Libraries/Libraries` (and `SharedLibraries`, or `Interfaces and Libraries/Libraries/Shared Libraries` if your kit spells it with spaces) under the Retro68 root. CMake also searches a sibling `InterfacesAndLibraries` tree beside the toolchain root (for example `/home/.../Retro68/InterfacesAndLibraries`) and an additional nested `Retro68/InterfacesAndLibraries` folder; if your Speech.a/SpeechSynthesis.a lives somewhere else, set `SPEECH_IMPORT_LIB_DIR` to that folder.
