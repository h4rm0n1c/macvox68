...
The `.r` header includes `Types.r`, `Finder.r`, and `Icons.r` so the Rez tool can see the templates for `icl8`/`ics8`, `ICN#`/`ics#`, and the `BNDL`/`FREF` bundle mappings without requiring extra hand edits. Bundle entries are emitted with the classic semicolon-delimited mapping syntax Retro68's Rez expects to avoid array parsing failures.
