...
The `.r` header includes `Types.r`, `SysTypes.r`, `Icons.r`, and `Finder.r` so the Rez tool can see the templates for `icl8`/`ics8`, `ICN#`/`ics#`, and the `BNDL`/`FREF` bundle mappings without requiring extra hand edits.
