...
The `.r` header includes `Types.r`, `Finder.r`, and `Icons.r` so the Rez tool can see the templates for `icl8`/`ics8`, `ICN#`/`ics#`, and the `BNDL`/`FREF` bundle mappings without requiring extra hand edits. Bundle entries now include the expected type counts and per-type ID counts from Finder.r, which avoids Retro68 Rez array assertions when compiling the bundle.
...
- Bundles follow the standard Finder mapping: the bundle declares five mapped types so the bundle type count aligns with Finder.r, each mapped type declares one local ID (`0`) pointing at the shared icon resource ID, and the `FREF` uses the same resource ID as the bundle. This keeps Retro68 Rez happy while still exposing the color and B&W icon variants for small and large sizes.
