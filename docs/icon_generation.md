...
- `icl8` and `ics8` store raw palette indices in row-major order and **must share the exact same palette** (the script enforces this). If the source palette looks like Photoshop’s “Macintosh” table (black at index 0, white at 255), the generator reverses the indices so the icon matches the QuickDraw/Finder expectation of white at index 0/black at 255. The script derives matching 4-bit `icl4`/`ics4` data from the first 16 palette entries with the same reversal so nibble indices stay aligned to the classic Mac 4-bit color table ordering.
